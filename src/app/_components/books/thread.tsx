...
const BookThread = () => {
  ...
  const { data: threads, refetch: refetchThreads } = api.bookThread.getThreads.useQuery({ isbn });
  ...
  return (
    <div className="mt-8">
      <Card className="bg-gray-800 text-gray-100 border-none shadow-lg mb-6">
        <CardHeader>
          <CardTitle className="text-xl font-bold text-blue-300">新しいスレッドを作成</CardTitle>
        </CardHeader>
        <CardContent>
          <Input
          ...
          <Textarea
            ...
            className="mb-4 bg-gray-700 text-gray-100 border-gray-600"
          />
          <Button onClick={handleCreateThread} className="bg-blue-600 hover:bg-blue-700 text-white">
      ...
      {threads?.map((thread) => (
        <Card key={thread.id} className="bg-gray-800 text-gray-100 border-none shadow-lg mb-6">
          <CardHeader>
            <CardTitle className="text-lg font-bold text-blue-300">{thread.title}</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div className="mt-4 flex">
              ...
              />
              <Button onClick={() => handleCreateComment(thread.id)} className="bg-green-600 hover:bg-green-700 text-white">
